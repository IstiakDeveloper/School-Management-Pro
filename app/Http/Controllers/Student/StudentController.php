...
class StudentController extends Controller
{
    ...
    private function autoGenerateStudentFees(Student $student)
    {
        try {
            ...
            // Exclude monthly fees (handled by auto-generation middleware)
            // Exclude quarterly fees (Exam Fee - will be generated when exam is created)
            $feeStructures = FeeStructure::with('feeType')
                ->where('class_id', $student->class_id)
                ->where('academic_year_id', $student->academic_year_id)
                ->whereHas('feeType', function($q) {
                    // Only include one-time fees like admission fee
                    // Exclude monthly fees (auto-generated by middleware)
                    // Exclude quarterly fees (Exam Fee - generated when exam is created)
